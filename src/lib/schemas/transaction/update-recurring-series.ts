...
/** Reuses the create-transaction error copy — same fields (minus `date`), since editing a series' template is conceptually the same shape. */
const __namespace = 'common.forms.transactions.create' satisfies NestedKey

export const updateRecurringSeriesSchema = (t: ReturnType<typeof useTranslations<typeof __namespace>>) =>
	z
		.object({
			name: z
				.string()
...
export type UpdateRecurringSeriesInput = z.infer<ReturnType<typeof updateRecurringSeriesSchema>>
