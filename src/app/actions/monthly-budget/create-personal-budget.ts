...
const createPersonalMonthlyBudgetChain = createAction<{ householdId: string; input: CreateMonthlyBudgetInput }>()
	.use(withErrorBoundary(monthlyBudgetLogger, '[createPersonalMonthlyBudget]: {error}'))
	.use(withAuthorizedSession)
	.use(withActiveMembership((ctx) => ctx.householdId))
	.handler(async ({ householdId, input, membership }): Promise<CreatePersonalBudgetResult> => {
...
export async function createPersonalMonthlyBudget(
	...
	return createPersonalMonthlyBudgetChain({ householdId, input })
}
